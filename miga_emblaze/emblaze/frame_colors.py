...
def compute_colors(frames, ids_of_interest=None, scale_factor=1.0):
    ...
    for i in range(len(frames)):
        cur_distance_sample = frames[i].ids.tolist()
        frame_1_neighbors = frames[i].get_recent_neighbors()[cur_distance_sample]
        for j in range(len(frames)):
            cur_distance_sample = frames[j].ids.tolist()
            frame_2_neighbors = frames[j].get_recent_neighbors()[cur_distance_sample]
    ...
    neighbor_dists = [np.log(1 + frame.distances(frames[i].ids.tolist(), frames[i].ids.tolist()).flatten()) for frame in frames]
