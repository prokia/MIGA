...
class SelectionRecommender:
    ...
    def _make_neighbor_changes(self, idx_1, idx_2, filter_points=None):
        ...
        gained_ids, lost_ids = [[] for _ in range(2)]
        for i in range(len(filter_points or frame_1)):
            f1_neighbors_i = set(frame_1_neighbors[i]) if i < len(frame_1_neighbors) else set()
            f2_neighbors_i = set(frame_2_neighbors[i]) if i < len(frame_2_neighbors) else set()
            gained_ids.append(f2_neighbors_i - f1_neighbors_i)
            lost_ids.append(f1_neighbors_i - f2_neighbors_i)
    ...
    def _inner_change_score(self, ids, frame_1, frame_2):
        ...
        two frames.
        """
        l_list = [len(x) for x in [frame_1, frame_2]]
        new_ids = [[] for i in range(2)]
        for i in range(2):
            for idx in ids:
                if idx < l_list[i]:
                    new_ids[i].append(idx)
        return np.mean(inverse_intersection(frame_1.get_recent_neighbors()[new_ids[0]],
                                            frame_2.get_recent_neighbors()[new_ids[1]],
                                            List(ids),
                                            False))
