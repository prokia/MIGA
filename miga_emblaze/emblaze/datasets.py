...
class ColumnarData:
    ...
    def __init__(self, data, ids=None):
        ...
        for field, values in data.items():
            ...
            if length is None:
                length = len(values)
            assert length == len(values), "Field '{}' has mismatched length (expected {}, got {})".format(field, length,
                                                                                                          len(values))
    ...
    def concat(self, other):
        ...
        assert set(self.data.keys()) == set(
            other.data.keys()), "Cannot concatenate ColumnarData objects with different sets of fields"
        assert not (set(self.ids.tolist()) & set(
            other.ids.tolist())), "Cannot concatenate ColumnarData objects with overlapping ID values"
    ...
    def set_field(self, field, values):
        assert self.length == len(values), "Field '{}' has mismatched length (expected {}, got {})".format(field,
                                                                                                           self.length,
                                                                                                           len(values))
        self.data[field] = np.array(values)
...
class Embedding(ColumnarData):
    ...
    def concat(self, other):
        ...
        """
        assert set(self.data.keys()) == set(
            other.data.keys()), "Cannot concatenate Embedding objects with different sets of fields"
        assert not (set(self.ids.tolist()) & set(
            other.ids.tolist())), "Cannot concatenate Embedding objects with overlapping ID values"
    ...
    def project(self, method=ProjectionTechnique.UMAP, **params):
        ...
        if method == ProjectionTechnique.UMAP:
            import umap
            trans = umap.UMAP(**params).fit(hi_d)
            ...
            raise ValueError(
                "Unrecognized projection technique '{}'. Please choose from the constants listed in emblaze.ProjectionTechnique, or pass a callable (see method docstring).".format(
                    method))
    ...
    def from_json(cls, data, label=None, parent=None):
        ...
        return cls(mats, ids=ids, label=label, metric=metric, n_neighbors=n_neighbors, neighbors=neighbors,
                   parent=parent)
    ...
    def align_to(self, base_frame, ids=None, return_transform=False, base_transform=None, allow_flips=True):
        ...
        for factor in (FLIP_FACTORS if allow_flips else FLIP_FACTORS[:1]):
            ...
            if rmsd < min_rmsd:
                min_rmsd = rmsd
                transform = ~base_proj_scaler * matrix_to_affine(opt_rotation.as_matrix()) * Affine.scale(
                    *factor[:2]) * proj_scaler
                if return_transform:
...
class NeighborOnlyEmbedding(Embedding):
    ...
    def concat(self, other):
        ...
        assert isinstance(other,
                          NeighborOnlyEmbedding), "Cannot concatenate non-neighbor-only to neighbor-only Embedding"
        assert not (set(self.ids.tolist()) & set(
            other.ids.tolist())), "Cannot concatenate Embedding objects with overlapping ID values"
...
class EmbeddingSet:
    ...
    def project(self, method=ProjectionTechnique.ALIGNED_UMAP, align=True, **params):
        ...
        if method == ProjectionTechnique.ALIGNED_UMAP:
            import umap
            trans = umap.AlignedUMAP(**params).fit(hi_ds, relations=[
                self.embeddings[i].get_relations(self.embeddings[i + 1])
            ...
            lo_ds = [emb.copy_with_fields({Field.POSITION: lo_d}, clear_neighbors=True)
                     for emb, lo_d in zip(self.embeddings, trans.embeddings_)]
        elif callable(method):
        ...
        if method != ProjectionTechnique.ALIGNED_UMAP:
            ...
            return EmbeddingSet(lo_ds, align=align and not pre_aligned), trans
    ...
    def from_json(cls, data, parents=None):
        ...
        embs = [Embedding.from_json(frame, label=label, parent=parent) for frame, label, parent in
                zip(embs, labels, parents)]
