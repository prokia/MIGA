...
class Viewer(DOMWidget):
    ...
    def _select_stored_num_neighbors(self, embeddings):
        ...
        """
        num_points = sum(len(e) for e in embeddings)
        new_val = None
    ...
    def _get_filter_points(self, selection, in_frame=None):
        ...
        for frame in self.embeddings.embeddings if in_frame is None else [in_frame]:
            ...
            for s in selection:
                if s < len(frame):
                    cur_selection.append(s)
            filtered_points |= set(frame.get_ancestor_neighbors()[cur_selection][:,:self.numNeighbors].flatten().tolist())
