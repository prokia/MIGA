...
class MoleculeDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.dataset == 'phaseI':
            smiles_list, rdkit_mol_objs, labels = \
                _load_phaseI_dataset(self.raw_paths[0])
            
            print("processing")
            for i in range(len(smiles_list)):
                rdkit_mol = rdkit_mol_objs[i]
                if rdkit_mol != None:
                    print(i, 'smiles is valid')
                    ...
                    data_list.append(data)
                    data_smiles_list.append(smiles_list[i])
            print(len(data_smiles_list))


        elif self.dataset == 'phaseII':
            smiles_list, rdkit_mol_objs, labels = \
                _load_phaseII_dataset(self.raw_paths[0])
        ...
        elif self.dataset == 'phaseIII':
            smiles_list, rdkit_mol_objs, labels = \
                _load_phaseIII_dataset(self.raw_paths[0])
...
def _load_phaseI_dataset(input_path):
    """
    ...
    labels
    """
    input_df = pd.read_csv('dataset/phaseI/df.csv')

    smiles_list = input_df['smiless'].values
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(smiles_txt_to_lst(s)[0]) for s in smiles_list]
    ...
    preprocessed_smiles_list = [smiles_txt_to_lst(smiles_list[i])[0] if m != None else
                                None for i, m in enumerate(preprocessed_rdkit_mol_objs_list)]
    # preprocessed_rdkit_mol_objs_list = rdkit_mol_objs_list
    # preprocessed_smiles_list = smiles_list
...
def _load_phaseII_dataset(input_path):
    """
    ...
    input_df = pd.read_csv('dataset/phaseII/df.csv')
    print('original df length',len(input_df))
    
    smiles_list = input_df['smiless'].values
    rdkit_mol_objs_list = [AllChem.MolFromSmiles(smiles_txt_to_lst(s)[0]) for s in smiles_list]
    
    print('rdkit_mol_objs_list',len(rdkit_mol_objs_list))
...
def _load_phaseIII_dataset(input_path):
    """
    ...
    labels
    """
    input_df = pd.read_csv('dataset/phaseIII/df.csv')
